...
def breadthFirstSearch(problem):
    ...
    while not queue.isEmpty():
        ...
        for next_state, action, step_cost in problem.getNextStates(state):
            if next_state not in visited:
        ...
        if state not in visited:
            ...
            for next_state, action, step_cost in problem.getNextStates(state):
                if next_state not in visited:
                    ...
                    pq.push((next_state, path + [action], total_cost), total_cost)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    start_state = problem.getInitialState()
    start_cost = 0
    priority_q.push((start_state, [], start_cost), start_cost + heuristic(start_state, problem))

    while not priority_q.isEmpty():
        state, path, cost = priority_q.pop()
        ...
        if state not in visited:
            ...
            for next_state, action, step_cost in problem.getNextStates(state):
                if next_state not in visited:
                    ...
                    priority_q.push((next_state, path + [action], new_cost), priority)
